...
export const structureStageFn = inngest.createFunction(
  ...
  async ({ event, step }) => step.run("structure-propose", () => executeProposalStage(structureStageSpec(event.data.runId), { softAck: event.data.softAck, force: event.data.force })),
);
