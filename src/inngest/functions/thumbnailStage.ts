...
export const thumbnailStageFn = inngest.createFunction(
  ...
  async ({ event, step }) => step.run("thumbnail-propose", () => executeProposalStage(thumbnailStageSpec(event.data.runId), { softAck: event.data.softAck, force: event.data.force })),
);
