...
export const hookStageFn = inngest.createFunction(
  ...
  async ({ event, step }) => step.run("titles-propose", () => executeProposalStage(hookStageSpec(event.data.runId), { softAck: event.data.softAck, force: event.data.force, reason: event.data.reason })),
);
