...
// 최신 structure proposal에 수정본으로 새 selection 행 INSERT(상태 전이 없음). editSelectedTitle 미러(descriptor만 structure).
//   chosen_idx는 최신 selection의 값을 보존(없으면 0). edited_payload=수정본 단일 객체(approach+outline).
export async function editSelectedStructure(
  ...
  const desc = STAGE_DESCRIPTORS.structure;
  if (!(await stageIsConfirmed(supa, runId, desc.stage))) {
    throw new Error(`구성 손편집은 확정(${desc.stage} 선택 기록) 후에만 가능 — 아직 확정 전.`);
  }
