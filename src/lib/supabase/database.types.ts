...
type ExplanationAssets = { id: string; run_id: string; concept: string; kind: "number" | "analogy" | "comparison" | "case"; numeric_example: string | null; analogy: string | null; created_by: string | null; source_fact_id: string | null; math_verified: boolean | null; distortion_checked: boolean | null; used_in_script: boolean; landed_score: number | null; payload: Json | null; created_at: string }; // migration 30: +kind('comparison')/payload(jsonb nullable) 비교 자산; migration 31: +kind('case') 케이스 분기 자산(payload 재사용)
...
type StyleProfiles = { id: string; component_type: "title" | "thumbnail_copy" | "description" | "structure" | "analogy_style" | "title_owner_rules" | "thumbnail_owner_rules"; version: number | null; patterns: Json | null; status: ProfileStatus; created_at: string }; // migration 25: +structure(구다리 구성 패턴); migration 33: +analogy_style(유이 비유 기법 프로필); migration 34: +owner_rules(김짠부 직접 피드백 최우선 규칙 — title_owner_rules·thumbnail_owner_rules)
