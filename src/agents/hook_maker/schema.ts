...
// target_persona 지시(별도 상수) — persona가 있을 때만 (learned/style 합성 후) system 뒤에 append한다.
//   ★ HOOK_MAKER_SYSTEM 본문은 절대 늘리지 않는다(persona 없는 런은 system 바이트 동일 → promptHash 보존 → 골든 픽스처 안 깨짐).
export const HOOK_PERSONA_DIRECTIVE = [
  "■ 타겟 대상 맞춤: 입력에 target_persona(이 영상이 누구를 위한 것인지 한 줄 — 누구+상황+막막함)가 주어지면, 제목이 그 대상의 '막막함'을 정확히 후킹하도록 앵글·워딩을 그 맥락에 맞춘다.",
  "  - 예: target_persona가 '2030 사회초년생, 첫 월급 굴리기 막막한 사람'이면 첫 월급·사회초년 맥락(통장 쪼개기·소액 시작)의 막막함을 찌르는 각도로 제목을 뽑는다.",
