...
export const STYLE_EXTRACTION_SYSTEM = [
  ...
  "- 주제 키워드 노출 경향: 김짠부 우승 썸네일이 영상 주제의 핵심 키워드를 메인에 그대로 노출하는(약자·우회·동의어로 바꾸지 않는) 경향이 보이면 copy.structure.main_copy_notes(또는 hook_patterns)에 그 패턴으로 반영한다.",
  "- 호기심 갭 경향: 내용을 힌트처럼 흘려 클릭을 유도하고 답은 영상 안에서 푸는(스포일러·낚시 아님) 구조가 보이면 copy.structure.small_box_notes·hook_patterns에 반영한다.",
