...
//   리서치 라이프사이클 4단계: 후보 선택 → 검증 실행 → 검수 → 완료.
...
const PHASES = [
  { label: "후보 선택", crew: "셜록이 검증 후보 제시 → 내가 선택" },
  { label: "검증 실행", crew: "팩트검증·교차정리·숫자/비유·반론" },
  { label: "검수", crew: "고위험 사실만 사람 확인" },
...
function phaseOf(state: RunState): { idx: number; working: boolean; allDone: boolean } {
  switch (state) {
    case "structure_selected":
      return { idx: 0, working: false, allDone: false }; // 리서치 시작 전(후보 선택 진입 대기)
    case "research_scoped":
      return { idx: 0, working: true, allDone: false };
    case "researching":
      return { idx: 1, working: true, allDone: false };
    case "research_ready":
      return { idx: 2, working: false, allDone: false }; // 검수 진입 대기
      ...
      <p className="mt-2 border-t border-trus-white/10 pt-2 text-[11px] text-trus-white/55">
        {allDone ? (
          ...
          <>
            ...
            {working ? " 진행 중" : " 대기"} · 남은 단계 {PHASES.length - idx - 1}개
          </>
        )}
      </p>
    </div>
