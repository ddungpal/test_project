...
describe("슬롯 재생성 변주 — buildRegenerateAugmentedSystem 재사용으로 promptHash 차등", () => {
  ...
  it("reason 전달 시 augmented system에 사유가 포함된다", () => {
    const reason = "텍스트가 너무 길어서 줄여줘";
    const aug = buildRegenerateAugmentedSystem(base, priors, 1, reason);
    expect(aug).toContain(reason);
    expect(aug).toContain("사용자 요청 이유");
  });

  it("reason 미전달이면 기존(reason 없는)과 바이트 동일 — 출력 불변·픽스처 보존", () => {
  ...
  it("reason 공백/빈문자면 출력 불변 — 사유 미반영(픽스처 보존)", () => {
    const withoutArg = buildRegenerateAugmentedSystem(base, priors, 1);
    expect(buildRegenerateAugmentedSystem(base, priors, 1, "")).toBe(withoutArg);
    expect(buildRegenerateAugmentedSystem(base, priors, 1, "   ")).toBe(withoutArg);
  });

  it("reason 유무로 promptHash가 차등된다(record/replay에서 새 결과)", () => {
    const augNo = buildRegenerateAugmentedSystem(base, priors, 1);
    const augYes = buildRegenerateAugmentedSystem(base, priors, 1, "더 강렬하게");
    const hNo = promptHash({ ...hashBase, system: augNo });
    const hYes = promptHash({ ...hashBase, system: augYes });
    expect(hYes).not.toBe(hNo);
  });
});
