...
interface FakeOptions {
  ...
  typeEqLog?: string[]; // corpus_components에서 eq("type", ...) 인자를 기록(레퍼런스 출처 검증용)
}

/** 테이블명으로 분기하는 체이너블 fake Supa. stage_proposals/selections는 stage(eq 인자)로 topic/title 분기. */
function makeFakeSupa({ styleRow, typeEqLog }: FakeOptions): Supa {
  const from = (table: string) => {
    ...
    chain.eq = (col: string, val: unknown) => {
      ...
      if (col === "type" && table === "corpus_components" && typeEqLog) typeEqLog.push(String(val));
      return chain;
    };
    chain.in = self;
    chain.order = self;
...
describe("prepareThumbnailMaker 배선(통합) — active 썸네일 스타일 주입", () => {
  ...
  it("케이스 C: 레퍼런스 corpus를 type='thumbnail_copy'로 쿼리한다(제목이 아니라 썸네일 문구)", async () => {
    const typeEqLog: string[] = [];
    const supa = makeFakeSupa({ styleRow: null, typeEqLog });
    const { input } = await prepareThumbnailMaker(supa, "run-C");
    expect(typeEqLog).toContain("thumbnail_copy");
    expect(typeEqLog).not.toContain("title");
    // rename된 필드가 존재(빈 배열이라도) — reference_titles는 더 이상 없음.
    expect(Array.isArray(input.reference_thumbnail_copies)).toBe(true);
  });
});
