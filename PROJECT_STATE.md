...
>
> ## ▶ 남은 작업 (BACKLOG · 2026-07-04 오후 갱신)
> **▶▶▶ 다음 재개점 = 짠펜 대본 품질 5규칙 라이브 검증**(2026-07-04 오후 세션) — 사용자가 실제 대본을 읽고 지적한 품질 문제를 **전부 프롬프트 강화로** 잡아 머지·push 완료(2 phase). **아직 안 본 것 = 새 대본 런을 리서치→스크립트까지 돌려, 5규칙이 실제 대본에 먹히는지 눈으로 확인**(프롬프트 변경이라 기존 런 소급X·promptHash 바뀌어 짠펜·유이 fixture 자동 재기록). 확인 목록:
