...
_Last updated: 2026-06-29(낮 — **제목 채널학습(title-channel-learn) phase 완료·main ff-머지(로컬)**. main tip=`d3ae83c`·test 633(+19)·typecheck 0·**origin push 안 함**. 어투=스크립트로만 유지 확정(자막 실험 후 폐기). 썸네일 학습은 v4로 이미 완료 확인. 라이브 dev에서 LiveRefresh realtime 채널명 충돌 버그 픽스됨) · 단계: **실사용 검증 + 학습 루프 강화. ▶▶ 다음(재개점) 2갈래: (1) 제목 채널학습 라이브 실행(claude-p $0, 코드 머지됨): `set -a; . ./.env; set +a` → `npx tsx scripts/ingest-channel-titles.ts @zzanboo --commit`(채널 최근 50개 제목 수집→corpus/titles/channel-recent.json) → `npx tsx scripts/extract-title-style.ts --commit`(제목 스타일 draft) → `/copy-learn` 제목 draft 검토·활성화 → 새 런으로 제목 품질 확인. (2) 단독 실행(standalone) 라이브 검증(아래 6-28 로그). + origin push(title-channel-learn). 후속: 구다리 3단계(성과기반 목차 랭킹)·⚠️OpenAI/구글 키 rotate(배포 전 최우선).**_
