...
> - **B. 사용자 액션(코드 아님)**: ⑥⚠️**마이그35 라이브 적용**(Supabase SQL: `insert into public.run_state_transitions (from_state,to_state) values ('approved','scripting') on conflict do nothing;` — 안 하면 "승인런 대본 다시 생성" 버튼이 트리거에 막힘) ⑦⚠️YouTube API 키 rotate(채팅 노출분) ⑧OpenAI 키 rotate.
> - **C. 제목/썸네일 고도화 계속(사용자 집중 요청)**: 1차=주제 근거 연결 완료. **다음 후보=앵글 확장·시그니처 강화·CTR/A/B 학습 활용** → "다음으로 아쉬운 점"을 사용자와 논의부터.
