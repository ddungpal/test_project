...
_Last updated: 2026-07-01(새벽 — **리서치 UX 대수술 진행 중: 게이트 5→1 자동화.** 브랜치 `feat-research-autoflow-pipeline`. **Phase 1 `research-autoflow-pipeline` 완료**(2 step·Joy PASS·test 941·마이그0), **Phase 2 `script-review-inline-facts` 설계 완료·아직 미실행**, Phase 3 미작성) · 단계: **▶▶▶ 다음(재개점) = `python3 scripts/run.py script-review-inline-facts` 실행**(Phase 2 step0·step1 이미 작성됨). 그 후 Phase 3 작성(게이트 UI 제거). ⚠️**거버넌스 일시공백**: Phase 1로 리서치가 사람 검수 없이 자동통과하는데 Phase 2(최종검수 인라인) 전엔 사람 최종확인이 비어있음 — Phase 2 먼저 끝내야 함. 후속(보류): 라이브 일괄 검증(copy-question-register 등 미완)·구다리 3단계·⚠️OpenAI/구글 키 rotate.**_
>
...
> **설계 전문**: `docs/specs/2026-07-01-research-autoflow-design.md` (커밋 8ac65c4). **3 phase 분해**: 1=백엔드 자동흐름 ✅, 2=최종검수 인라인 사실 ⏸설계만, 3=게이트 UI 제거 (미작성).
...
> - **⏸ Phase 2 `script-review-inline-facts`**(step0·step1 **작성 완료·미실행** — `phases/script-review-inline-facts/`): step0=`scriptView`에 세그먼트별 fact 상세+pending, 신규 `reviewScriptAction(runId,{rejectFactIds})`(보류 비반려→human_approved=true=거버넌스landing·반려→false+`requestScriptRework` 전체재작성). step1=`ScriptReview`에 인라인 칩(`FactCard` control 슬롯 재사용)+반려토글(기본 승인)+출처, Esther 투입. **▶ 다음 액션: 이 phase 실행.**
> - **⬜ Phase 3 `research-gate-removal`**(미작성): `runs/[id]/page.tsx`에서 죽은 게이트(`ResearchScopeGate`·`EnterReviewButton`·중간버튼) 제거, `ResearchPhaseStepper` 갱신, 시작버튼·재진입(고급) 잔존. Phase 2 실행 후 결과 보고 작성.
>
