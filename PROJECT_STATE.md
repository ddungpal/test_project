...
>
> ## ▶ 남은 작업 (BACKLOG · 2026-07-04 갱신)
> **▶▶▶ 다음 재개점 = 리서치 단계부터 비유 학습 검증**(사용자 2026-07-04 세션종료 시 지정) — analogy-learning 완료·머지·push·라이브검증(전사→추출→활성화)·이중가드까지 끝. **아직 안 본 것 = 새 대본 제작 런을 리서치 단계까지 돌려, 유이(analogist)가 실제로 (a)학습한 기법을 적용하고 (b)'바나나 마을' 등 레퍼런스 소재를 베끼지 않고 새 비유를 만드는지** 확인(v2 active·리서치 셀 `analogyStep` 경로·`needs_analogy` 개념에 붙음). 서버 기동중(dev:3000·inngest:8288·재기동은 **inngest 먼저→dev**·`.next` 정리·실행 중 `.next` 지우지 말 것). + **비유 코퍼스 확장**: 현재 표본=경제/화폐 7개(confidence tentative)라, 다른 도메인(건강·심리 등) 비유 릴스 mp4 추가→`/copy-learn` 재학습→활성화하면 유이 응용범위 넓어짐.
