...
>
> **다음 기획 — 진행 확정(2026-07-02·사용자 결정: 1·2·3 적용, 구 4번 구다리3단계 삭제):**
> 1. **리서치·제목/썸네일에 타겟 페르소나 주입** — target-persona 보류 후속. 페르소나가 리서치할 사실·후킹까지 좌우. (효과 크고 자연스러운 연장·추천 우선)
> 2. **쏙이 금맥을 훅이·셜록에도 주입** — 온보딩 부산물(금맥)을 구다리 넘어 제목/썸네일(훅이)·리서치(셜록)까지 조건부 주입. (1번과 downstream 컨텍스트 주입으로 합류 가능)
> 3. **"타겟 먼저" 모드** — 김짠부가 타겟 먼저 고정→그 타겟용 주제만 발굴 (target-persona 설계 (A)안).
