...
_Last updated: 2026-07-01(낮 — **리서치 UX 대수술: 게이트 5→1 자동화.** 브랜치 `feat-research-autoflow-pipeline`. **Phase 1 완료**(2 step·test 941·마이그0), **Phase 2 `script-review-inline-facts` 완료**(2 step·Max+Esther+Joy 1라운드 PASS·test 955·마이그0·떠돌이0·feat a5ec2e4+4f9786a), **Phase 3 미작성**) · 단계: **▶▶▶ 다음(재개점) = Phase 3 `research-gate-removal` 작성·실행**(게이트 UI 제거). ✅**거버넌스 공백 해소**: Phase 2로 최종 스크립트 검수에 인라인 사실 칩+보류 fact 승인/반려가 들어와 사람 최종확인 1회 복구(보류 fact는 사람 액션에서만 human_approved=true·반려=전체 재작성). 후속(보류): 라이브 일괄 검증(copy-question-register 등 미완)·구다리 3단계·⚠️OpenAI/구글 키 rotate. **push·main 머지 안 함.**_
>
...
> **설계 전문**: `docs/specs/2026-07-01-research-autoflow-design.md` (커밋 8ac65c4). **3 phase 분해**: 1=백엔드 자동흐름 ✅, 2=최종검수 인라인 사실 ✅, 3=게이트 UI 제거 (미작성).
...
> - **🟡 Phase 3 `research-gate-removal`**(2 step **설계 완료·미실행** — `phases/research-gate-removal/`·**다음 재개점=실행**): 진단=Phase1·2 후 happy-path에서 사용자가 여전히 누르는 잉여 클릭은 `script_ready`의 "대본 검수 시작" 하나뿐이고, 리서치 게이트 4개는 Inngest 한 invocation 안에서 자동 통과돼 **전이 중 잠깐만 스치는 죽은 UI**. **step0 `script-review-landing`**(파이프라인)=`scriptStage` 끝에 `s1.state==='script_ready'`일 때만 `enter-script-review` step.run 추가(`enterScriptReview` 멱등 재사용·rework/aborted/non-ok 제외) → 런이 검수화면(`script_review`)에 **바로 착지**·마지막 잉여 클릭 제거. **step1 `research-gate-cleanup`**(UI·Esther)=`page.tsx`의 죽은 게이트(`ResearchScopeGate`·`EnterReviewButton`·`ResearchReview`·"대본 작성 시작"·`EnterScriptReviewButton`)를 `WaitingNote`로 교체, `getResearchScopeView`/scope조회·죽은 import 정리, `ResearchPhaseStepper` 문구 자동흐름 반영. **보존**: 시작버튼(structure_selected)·진행마커·`ScriptReview`(script_review)·완료뷰·`ResearchPanel` 미리보기. **사용자 결정(A)**: 재진입(`ResearchReentryActions`)은 happy-path 드랍·**컴포넌트 파일은 삭제 안 함**(미노출·코드만 잔존·고급옵션 부활 여지). 마이그 0.
>
