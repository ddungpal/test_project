...
_Last updated: 2026-06-29(낮4 — **리서치 선택 게이트(research-scope-gate) phase 완료·main ff-머지(로컬)**. main tip=`d7cc191`·test 668(+17)·typecheck 0·**origin push 안 함**·**⚠️마이그레이션 27 미적용**) · 단계: **실사용 검증 + 학습 루프 강화. ▶▶ 다음(재개점): (0) ⚠️**마이그레이션 `20260629120027_research_scoped_stage.sql` Supabase 적용**(research 게이트 동작 선결) + origin push. (1) 리서치 선택 게이트 라이브 검증(dev): 구성 확정 후 리서치 시작 → `research_scoped`에서 셜록 후보(섹션별·금융⚠️) 체크 선택 → "리서치 시작" → 선택분만 검증되는지. 셜록 fixture 6개는 첫 라이브 풀런 때 자동 재기록(claude-p $0). (2) 채널 제목학습 버튼·썸네일 비차단 큐 검증. (3) 단독 실행 검증. 후속: 구다리 3단계·⚠️OpenAI/구글 키 rotate(배포 전 최우선).**_
