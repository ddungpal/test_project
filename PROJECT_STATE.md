...
>
> **다음 기획 후보 (리서치·타겟 흐름 연장·추천순):**
> 1. **리서치·제목/썸네일에 타겟 페르소나 주입** — target-persona 보류 후속. 페르소나가 리서치할 사실·후킹까지 좌우. (효과 크고 자연스러운 연장·추천)
> 2. **"타겟 먼저" 모드** — 김짠부가 타겟 먼저 고정→그 타겟용 주제만 발굴 (target-persona 설계 (A)안).
> 3. **구다리 3단계** — 목차를 성과(CTR) 기반 랭킹 (단, 성과 데이터 쌓여야 의미 — 지금 데이터 부족).
