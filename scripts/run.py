...
사용법:
    python3 scripts/run.py <phase-dir>            # 현재 터미널에서 라이브 대화창
    python3 scripts/run.py <phase-dir> --window   # 새 Terminal.app 창을 열어 거기서 실행(macOS)
...
def _open_in_terminal(phase: str) -> int:
    """새 Terminal.app 창을 열고 거기서 (윈도우 없이) run.py를 다시 실행한다 (macOS).

    한글·공백 경로 안전: 셸 인자는 shlex.quote(작은따옴표)로 감싸고, 그 문자열을
    AppleScript 큰따옴표 리터럴에 넣는다(작은따옴표는 escape 불필요). 만일을 위해
    AppleScript 메타문자(\\, ")만 한 번 더 escape한다.
    ponytail: macOS Terminal.app 전용. iTerm/Linux면 사용자가 직접 run.py 실행.
    """
    inner = (
        f"cd {shlex.quote(str(ROOT))} && "
        f"{shlex.quote(sys.executable)} {shlex.quote(str(SCRIPTS / 'run.py'))} {shlex.quote(phase)}"
    )
    ...
    return subprocess.run(["osascript", "-e", osa]).returncode
...
def main():
    ...
    rest = [a for a in args if a != "--window"]
    if not rest:
        print("사용법: python3 scripts/run.py <phase-dir> [--window]", file=sys.stderr)
        sys.exit(1)
    phase = rest[0]
    ...
    if not phase_dir.is_dir():
        ...
        sys.exit(1)
    if window:  # 새 Terminal 창에서 다시 실행(진짜 tty → 컬러 라이브)
        sys.exit(_open_in_terminal(phase))
